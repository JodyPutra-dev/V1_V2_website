...
// V2 AutoData categorical indexes for optimized queries
...
// VERSION 2: Request queue limits concurrent predictions per instance
// maxConcurrent: 3 — at most 3 predictions active at once (1 runs in Python, 2 queue in PythonWorkerPool)
// maxWaiting: 10   — at most 10 more may wait in the Node.js queue before 503 is returned
// Total per-instance capacity: 13 in-flight predictions; 14th gets an immediate 503
// With 2 ML instances in cluster mode, total system capacity: 26 in-flight predictions
// This prevents unbounded backlog buildup that causes gradual event-loop saturation under sustained load
...
// Create a Python bridge for prediction
...
async function initializeWorkerPool() {
  try {
    ...
    const modelType    = modelPath.endsWith('.pkl') ? 'pkl' : 'joblib';
...
function checkDehydrationFromRGB(red = 255, green = 220, blue = 150) {
  ...
  let hydrationStatus, needsWater, recommendation;
...
async function predictWithJoblib(data) {
  try {
    const inputData = preprocessInput(data);
    ...
    if (!result || !result.success) {
      throw new Error(result?.error || 'Invalid prediction result from Python worker');
    }
...
app.post('/predict/csv', upload.single('csv'), async (req, res) => {
  try {
    ...
    // VERSION 2: Process CSV rows with queue control
    // Each row acquires a queue slot, preventing 100 simultaneous Python processes
    const predictions = [];
    for (const row of data) {
      await predictionQueue.acquire();
      console.log('[ML-QUEUE] Acquired prediction slot for CSV row', { 
        queueStats: predictionQueue.getStats() 
      });
      ...
      try {
        ...
        predictionQueue.release();
        console.log('[ML-QUEUE] Released prediction slot for CSV row', { 
          queueStats: predictionQueue.getStats() 
        });
      }
    }
...
async function startServer() {
  try {
    // Initialize everything BEFORE binding the HTTP port so that no request
    // can arrive while the worker pool is still null.
    await ensureMongoDBConnection();
    await initialize();
    console.log('ML service initialization completed successfully');
    await initializeWorkerPool();

    // Only now bind the HTTP port — at this point the worker pool is ready.
    return new Promise((resolve, reject) => {
      const server = app.listen(PORT, () => {
        console.log(`ML service running on port ${PORT}`);

        // Send PM2 ready signal after port is bound AND pool is ready.
        if (process.send) {
          process.send('ready');
