...
const HTTPS_PORT = parseInt(process.env.HTTPS_PORT || 443, 10); // HTTPS port
...
const DEPLOYMENT_VERSION = process.env.DEPLOYMENT_VERSION || 'V1-NON-NGINX-BASELINE';
...
// Log large request bodies — common debugging pattern developers add when
// tracking down payload issues. Winston is async so this does not block.
app.use((req, res, next) => {
  if (req.body && Object.keys(req.body).length > 0) {
    const bodySize = JSON.stringify(req.body).length;
    if (bodySize > 1024) {
      logger.info(`Large request body: ${bodySize} bytes on ${req.method} ${req.url}`, {
        bodySize,
        method: req.method,
        url: req.url,
        instanceId: INSTANCE_ID
...
// Indexes for new categorical parameters (turbidity and color classification)
...
app.use('/api/predict', async (req, res) => {
  try {
    logger.info('Prediction proxy request', {
      method: req.method,
      url: req.url,
    ...
    if (headers['content-type']?.includes('multipart/form-data')) {
      ...
      const requestId = `csv-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
      ...
      const upload = multer({ 
        storage: multer.diskStorage({
          destination: function (req, file, cb) {
            ...
            logger.info('Temp directory created', { requestId, tempDir });
            cb(null, tempDir);
          },
          filename: function (req, file, cb) {
            const filename = Date.now() + '-' + file.originalname;
            logger.info('Saving uploaded file', { requestId, filename });
            cb(null, filename);
          }
        }),
        fileFilter: function (req, file, cb) {
      ...
      upload(req, res, async (err) => {
        ...
        try {
          if (!req.file) {
            logger.warn('No file in CSV request', { requestId });
            return res.status(400).json({
              success: false,
          ...
          logger.info('File saved in temp', {
            requestId,
            path: req.file.path,
            size: req.file.size,
            mimetype: req.file.mimetype,
          ...
          if (req.file.size === 0) {
            logger.warn('Empty CSV file uploaded', {
              requestId,
          ...
          logger.info('Created FormData for forwarding', {
            requestId,
          ...
          logger.info('Forwarding CSV to prediction service', {
            requestId,
            url
          });
          ...
          logger.info('Prediction service response', {
            requestId,
            status: predictionResponse.status,
            success: responseData.success,
            resultCount: responseData.data?.results?.length || responseData.results?.length || 0,
            errorCount: responseData.data?.errors?.length || responseData.errors?.length || 0,
            message: responseData.message
          });
          
          // Log error response body snippet if error
          if (predictionResponse.status >= 400) {
            logger.warn('Prediction service error response', {
              requestId,
              status: predictionResponse.status,
              bodySnippet: JSON.stringify(responseData).substring(0, 500)
            });
          }
          ...
          if (req.file && fs.existsSync(req.file.path)) {
            try {
              fs.unlinkSync(req.file.path);
              console.log(`${new Date().toISOString()}: [PREDICTION-PROXY] [${requestId}] Cleaned up temp file on error: ${req.file.path}`);
            } catch (cleanupError) {
              console.error(`${new Date().toISOString()}: [PREDICTION-PROXY] [${requestId}] Error cleaning up temp file:`, cleanupError);
            }
          }
          
          console.error(`${new Date().toISOString()}: [PREDICTION-PROXY] [${requestId}] Error:`, error.message);
          return res.status(500).json({
            success: false,
...
app.use('/api/ml', async (req, res) => {
  try {
    ...
    // Continue with the rest of the code...
      ...
      console.log('[ML-PROXY] Forwarding /autodata GET with headers:', headers);
      console.log('[ML-PROXY] Query params:', req.query);
      
      const url = `http://localhost:${ML_SERVICE_PORT}${req.url}`;    const options = {
      method: req.method,
      headers,
      body: req.method !== 'GET' && req.method !== 'HEAD' ? JSON.stringify(req.body) : undefined
    };
...
// Prediction middleware - forwarding to prediction service
app.use('/api/predict', async (req, res) => {
  try {
    console.log(`${new Date().toISOString()}: [PREDICTION-PROXY] Forwarding request: ${req.method} ${req.originalUrl}`);
    
    const headers = { ...req.headers };
    delete headers.host;
    
    // Build target URL (req.url already strips /api/predict prefix)
    const url = `http://localhost:${PREDICTION_SERVICE_PORT}${req.url}`;
    
    console.log(`${new Date().toISOString()}: [PREDICTION-PROXY] Target URL: ${url}`);
    
    // Handle multipart/form-data (CSV uploads)
    if (req.method === 'POST' && req.headers['content-type']?.includes('multipart/form-data')) {
      ...
      }).single('csv'); // Field name must match frontend: formData.append('csv', file)
      ...
      upload(req, res, async (err) => {
        ...
        try {
          ...
          // Retry logic for CSV uploads
          const maxRetries = 3;
          const baseDelay = 150; // 150ms, 300ms, 600ms
          let lastError = null;
          
          for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
              console.log(`${new Date().toISOString()}: [PREDICTION-PROXY] Attempt ${attempt}/${maxRetries} for ${url}`);
              
              // Forward the file to prediction service using fetch and FormData
              const formData = new FormData();
              formData.append('csv', fs.createReadStream(req.file.path), {
                filename: req.file.originalname,
              ...
              if (headers['user-id']) {
                fetchHeaders['user-id'] = headers['user-id'];
              }
              if (headers['x-request-id']) {
                fetchHeaders['x-request-id'] = headers['x-request-id'];
              }
              
              // Send to prediction service with 60s timeout for large CSVs
              const controller = new AbortController();
              ...
              if (!predictionResponse.ok) {
                ...
                console.error(`${new Date().toISOString()}: [PREDICTION-PROXY] Prediction service error (${predictionResponse.status}):`, errorData);
                ...
                if (predictionResponse.status >= 400 && predictionResponse.status < 500) {
                  return res.status(predictionResponse.status).json({
                    ...
                    error: errorData.error || predictionResponse.statusText
                  });
                }
                
                // Retry 5xx server errors
                throw new Error(`Prediction service error: ${predictionResponse.status} ${predictionResponse.statusText}`);
              }
              
              const result = await predictionResponse.json();
              console.log(`${new Date().toISOString()}: [PREDICTION-PROXY] Success: ${result.data?.processed || 0} predictions processed`);
              return res.json(result);
              
            } catch (attemptError) {
              lastError = attemptError;
              console.error(`${new Date().toISOString()}: [PREDICTION-PROXY] Attempt ${attempt} failed:`, attemptError.message);
              
              if (attempt < maxRetries) {
                const delay = baseDelay * Math.pow(2, attempt - 1);
                console.log(`${new Date().toISOString()}: [PREDICTION-PROXY] Waiting ${delay}ms before retry...`);
                await new Promise(resolve => setTimeout(resolve, delay));
              }
            }
          }
          ...
          try {
            ...
            console.error(`${new Date().toISOString()}: [PREDICTION-PROXY] Final cleanup error:`, cleanupError);
          }
          ...
          try {
            ...
            console.error(`${new Date().toISOString()}: [PREDICTION-PROXY] Cleanup error:`, cleanupError);
          }
      ...
      // Handle JSON requests (single predictions, history, etc.)
      const maxRetries = 3;
      const baseDelay = 150; // 150ms, 300ms, 600ms
      ...
      for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
          ...
          // Add timeout (30s for JSON requests)
          const controller = new AbortController();
          const timeout = setTimeout(() => controller.abort(), 30000);
          fetchOptions.signal = controller.signal;
          
          const predictionResponse = await fetch(url, fetchOptions);
          clearTimeout(timeout);
          ...
          const result = await predictionResponse.json();
          console.log(`${new Date().toISOString()}: [PREDICTION-PROXY] Success: ${req.method} ${req.url}`);
          return res.json(result);
...
const startServers = () => {
  ...
  try {
    ...
    // SSL_KEY_FILE / SSL_CERT_FILE env vars always take priority (set in .env.v1)
    let sslOptions = null;
    const certPaths = [
      // Explicit env-var override — highest priority, used for production domain cert
      {
        key: process.env.SSL_KEY_FILE || '',
        cert: process.env.SSL_CERT_FILE || ''
      },
      // Project-local Let\'s Encrypt / domain cert (ssl/ directory)
      {
      ...
      // Default Ubuntu self-signed cert paths (last resort — wrong CN for public domain)
      {
